...
import { Car, ClipboardList, LayoutGrid, Plus, Settings, Users, UsersRound } from "lucide-react";
...
function items(role: Role): Item[] {
  ...
  if (hasPermission(role, "garage:settings")) list.push({ href: "/app/parametres", label: "Paramètres", icon: Settings });
  return list;
}
...
export function AppNav({ role }: { role: Role }) {
  ...
  const canCreate = hasPermission(role, "workorders:create");
  return (
...
export function MobileNav({ role }: { role: Role }) {
  const pathname = usePathname();
  const canCreate = hasPermission(role, "workorders:create");
  const list = items(role).slice(0, 4);
  return (
